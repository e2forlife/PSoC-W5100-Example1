 ...
 */
#define W5100_SpiDone     (SPIM_ReadTxStatus() & (SPIM_STS_SPI_DONE | SPIM_STS_SPI_IDLE))
/* ------------------------------------------------------------------------ */
/**
...
void W5100_W51_Write(uint16 addr, uint8 dat)
{
	...
	SPIM_WriteTxData(W5100_WRITE_OP);
	SPIM_WriteTxData((addr>>8)&0x00FF);
	SPIM_WriteTxData(addr&0x00FF);
...
uint8 W5100_W51_Read(uint16 addr)
{
	...
	SPIM_ClearRxBuffer();
	/* Begin data read */
	SPIM_WriteTxData(W5100_READ_OP);
	SPIM_WriteTxData(addr>>8);
	SPIM_WriteTxData(addr&0x00FF);
	SPIM_WriteTxData( 0 );
...
#define W5100_SpiDone    (SPIM_ss0_m_Read())
#elif (0 == 1)
#include <SPIM_ss1_m.h>
#define W5100_SpiDone    (SPIM_ss1_m_Read())
#elif (0 == 2)
#include <SPIM_ss2_m.h>
#define W5100_SpiDone    (SPIM_ss2_m_Read())
#elif (0 == 3)
#include <SPIM_ss3_m.h>
#define W5100_SpiDone    (SPIM_ss3_m_Read())
#else
#include <SPIM_ss0_m.h>
#define W5100_SpiDone    (SPIM_ss0_m_Read())
#endif
...
{
#if (0 == 0)
	SPIM_SpiSetActiveSlaveSelect( SPIM_SPIM_ACTIVE_SS0 );
#elif (0 == 1)
	SPIM_SpiSetActiveSlaveSelect( SPIM_SPIM_ACTIVE_SS1 );
#elif (0 == 2)
	SPIM_SpiSetActiveSlaveSelect( SPIM_SPIM_ACTIVE_SS2 );
#elif (0 == 3)
	SPIM_SpiSetActiveSlaveSelect( SPIM_SPIM_ACTIVE_SS3 );
#else
		SPIM_SpiSetActiveSlaveSelect(0);
#endif
}
/* ------------------------------------------------------------------------ */
/**
...
void W5100_W51_Write(uint16 addr, uint8 dat)
{
	...
	/* Begin transfer of data to device */
	SPIM_SpiUartWriteTxData(W5100_WRITE_OP);
	SPIM_SpiUartWriteTxData((addr>>8)&0x00FF);
	SPIM_SpiUartWriteTxData(addr&0x00FF);
	SPIM_SpiUartWriteTxData(dat);
}
/* ------------------------------------------------------------------------ */
/**
 * \brief Read data from the W5100 at the spsecified address
...
uint8 W5100_W51_Read(uint16 addr)
{
	...
	SPIM_SpiUartClearRxBuffer();
	/* Begin data read */
	SPIM_SpiUartWriteTxData(W5100_READ_OP);
	SPIM_SpiUartWriteTxData(addr>>8);
	SPIM_SpiUartWriteTxData(addr&0x00FF);
	SPIM_SpiUartWriteTxData( 0 );
	/* Wait for operations to complete. */
	/* V1.1: Wait for SPI operation to complete */
	while( W5100_SpiDone == 0) {
		CyDelayUs(1);
	...
	count = SPIM_SpiUartGetRxBufferSize();
	while( count > 0 )
	{
		dat = SPIM_SpiUartReadRxData(); 
		count = SPIM_SpiUartGetRxBufferSize();
	}
